...
class TimelinesTests: XCTestCase {
    ...
    func testHomeTimelineWithRange() {
        let request = MastodonRequests.Timelines.Home(range: .limit(32))
    ...
    func testPublicTimelineFederated() {
        let request = MastodonRequests.Timelines.Public()
    ...
    func testPublicTimelineLocalWithRange() {
        let request = MastodonRequests.Timelines.Public(local: true, range: .since(id: "420", limit: 12))
    ...
    func testTagTimelineLocal() {
        let request = MastodonRequests.Timelines.Tag("mastodonkit", local: true)
    ...
    func testTagTimelineFederated() {
        let request = MastodonRequests.Timelines.Tag("mastodonkit")
    ...
    func testTagTimelineWithRange() {
        let request = MastodonRequests.Timelines.Tag("mastodonkit", range: .since(id: "420", limit: 12))
        let expectedLimit = URLQueryItem(name: "limit", value: "12")
        let expectedSinceID = URLQueryItem(name: "since_id", value: "420")
